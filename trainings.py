...
def dict_training_multi_branch(num_epoch,
    ...
    for epoch in range(start_epoch, num_epoch + 1):
        ...
        loss_parts_train = [number / (idx + 1) for number in loss_parts_train]
        ##create a matrix of loss in each epoch for each part
        attr_loss_train[epoch, :] = torch.tensor(loss_parts_train).float()
        ...
        #calculte mean of loss for each iteration for each part
        loss_parts_test = [number / (idx + 1) for number in loss_parts_test]
        #create a matrix of loss in each epoch for each part
        attr_loss_test[epoch, :] = torch.tensor(loss_parts_test).float()
        ...
        #attr losses saving
        torch.save(attr_loss_train, os.path.join(saving_path, 'train_part_loss.pth'))
        torch.save(attr_loss_test, os.path.join(saving_path, 'test_part_loss.pth'))
